...
This application was built to showcase what I had learned about redux/redux-sagas and putting it into production. The premise is a movie list manager that provides details and allows the user to click a single title to view the movie on a separate view.
...
7. NOT APPLICABLE FOR THIS PROJECT - Create a `.env` file at the root of the project and paste this line into the file and change "superDuperSecret" to any random value: `SERVER_SESSION_SECRET=superDuperSecret`
8. Create a database named `saga_movies_weekend` in PostgresSQL
If you would like to name your database something else, you will need to change `saga_movies_weekend` to the name of your new database name in `server/modules/pool.js`
